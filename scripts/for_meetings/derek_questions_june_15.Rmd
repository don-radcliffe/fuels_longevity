...
How strongly do we want to try and draw conclusions from these types of graphs?  Obviously I'll couch any language I use, because these studies all have nuanced variation in treatment implementation, study design, ecological context, etc., that we can't really capture here.   

How in-depth do we present each Washington dataset, or do we just include it in the meta-analysis graph?  The latter may be the better option from the standpoint of trying to publish the data later. 
